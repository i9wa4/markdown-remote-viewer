{
  description = "markdown-remote-viewer - Local Markdown directory HTTP viewer";
        ...
        in
        {
          apps = {
            ...
            update-go-toolchain = {
              ...
              program = "${pkgs.writeShellScriptBin "update-go-toolchain" ''
                ...
                go_minor="$(printf '%s\n' "$override_go" | ${pkgs.coreutils}/bin/cut -d. -f1-2)"
                ...
                GO_VERSION="$latest" GO_HASH="$hash" ${pkgs.perl}/bin/perl -0pi -e '
          ...
          packages.default = buildGo126Module {
            ...
            ldflags = [
              "-s"
              "-w"
              "-X github.com/i9wa4/markdown-remote-viewer/internal/version.Version=${version}"
              "-X github.com/i9wa4/markdown-remote-viewer/internal/version.Commit=${commit}"
            ];
          };
